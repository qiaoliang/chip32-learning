...
#define MPU6500_MEM_REV_ADDR (0x17)
    ...
    int bypass_state = mpu_set_bypass(0);
    if (bypass_state)
        return bypass_state; // 修改返回
#endif
...
u8 mpu_dmp_init(void)
{
    ...
    int mpu_init_res = mpu_init(); // 初始化MPU6050
    if (mpu_init_res == 0)
    {
...
/**
 *  @brief  得到dmp处理后的数据(注意,本函数需要比较多堆栈)
 *
 *  @param[in]  pitch:俯仰角 精度:0.1°   范围:-90.0° <---> +90.0°
 *  @param[in]  roll:横滚角  精度:0.1°   范围:-180.0°<---> +180.0°
 *  @param[in]  yaw:航向角   精度:0.1°   范围:-180.0°<---> +180.0°
 *  @return     0 if successful, otherwise.
 */

...
float pitch, roll, yaw; // MPU显示的欧拉角
...
void EXTI15_10_IRQHandler(void)
{
	...
	{
		// measurement 测量值的取值与 MPU6050 的安装方向有关。我的安装方向是 Y轴（Pitch）与车体平行，所以使用 Pitch 。
		measurement = pitch;
		targetPoint = balance_angle; // 理论值

		velocity = (read_encoder2() + read_encoder3()) / 2; // 两个轮子的速度取平均值

		// PID计算：直立环+速度环
		PWM = vertical_PID_value(measurement, targetPoint)+ velocity_PID_value(velocity,0);

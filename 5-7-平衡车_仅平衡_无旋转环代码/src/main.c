...
extern float pitch, roll, yaw; // 欧拉角测量值，当前的代码中，我使用了 pitch ，
float balance_angle = -0.45;   // 小车平衡时的角度。这里是我自己的小车初始（平衡)角度，如果你的小车初始角度不是这个, 请自行调整。
...
/**
下面是我调整参数时在不同步骤使用的参数值，你的参数可能会与我的不同（因为实际的小车是不同的)。
...
int main()
{
	...
	********************************************************************************/
	...
	{
		// 【1】安全装置：
		if (pitch < -30 || pitch > 30) // 如果超过30度，小车可能已经倒了
		{
			motor_flag = 0;			 // 关闭电机

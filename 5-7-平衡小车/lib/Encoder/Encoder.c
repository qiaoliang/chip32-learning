...
void Encoder_Right_Init(void)
{
	...
	TIM_ICStructInit(&TIM_ICInitStructure); // 结构体初始化，若结构体没有完整赋值
											...
											// 避免结构体初值不确定的问题
	TIM_ICInitStructure.TIM_ICFilter = 10;				  // // 输入滤波器参数，可以过滤信号抖动
	TIM_ICInit(Encoder_Right_TIMER, &TIM_ICInitStructure); // 将结构体变量交给TIM_ICInit，配置TIM3的输入捕获通道
	...
	// 注意此时参数的Rising和Falling已经不代表上升沿和下降沿了，而是代表是否反相
	// 此函数必须在输入捕获初始化之后进行，否则输入捕获的配置会覆盖此函数的部分配置
	TIM_ITConfig(Encoder_Right_TIMER, TIM_IT_Update, ENABLE); // 配置溢出更新中断标志位
	TIM_SetCounter(Encoder_Right_TIMER, 0);					  // 清零定时器计数值
	/*TIM使能*/
